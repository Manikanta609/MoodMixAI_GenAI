...
\author{\IEEEauthorblockN{1\textsuperscript{st} Manikanta}
\IEEEauthorblockA{\textit{Dept. of Computer Science} \\
\textit{Your University/Institute}\\
City, Country \\
email@example.com}
}
...
The cropped face image ($224 \times 224 \times 3$) is divided into fixed-size patches (e.g., $16 \times 16$). Each patch is linearly embedded, position embeddings are added, and the resulting sequence of vectors is fed into a standard Transformer encoder. The model, pre-trained on ImageNet and fine-tuned on FER datasets, outputs a probability distribution over 7 classes:
\[ C_{CV} = \{Angry, Disgust, Fear, Happy, Sad, Surprise, Neutral\} \]

The self-attention mechanism in ViT allows the model to weigh the importance of different facial regions (e.g., eyes vs. mouth) dynamically, leading to higher accuracy in detecting subtle expressions.
...
\subsubsection{DistilBERT Model}
The tokenized sequence is processed by DistilBERT. The model consists of 6 Transformer layers (compared to BERT's 12), with 66 million parameters. It outputs a hidden state vector for the `[CLS]` token, which serves as the aggregate representation of the sentence. A classification head then maps this vector to 6 emotion classes:
